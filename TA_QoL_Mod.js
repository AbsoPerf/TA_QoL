var id = "theory_automator_qol_merged";
var name = "Theory Automator + QoL";
var description = "Automates purchases/publications in theories with fine auto-purchase controls and heuristic-based star/student reallocation.";
var authors = "rus9384 (TA)\nEaux Tacous (QoL)\nAbsoPerf (Integration)";
var version = "2.50_integrated";
var permissions = Permissions.PERFORM_GAME_ACTIONS;

// =====================================================================
// [1] 전역 변수 및 설정 선언 (두 스크립트 변수 병합)
// =====================================================================
var theoryManager;
var timer = 0;
...
// =====================================================================
// [2] MathExpUtils (QoL 수식 해석 유틸리티 클래스)
// =====================================================================
class MathExpUtils {
    constructor(globalMap, globalOrder, theoryMap, theoryOrder) {
        ...
        this.targetMap = (target) => {
            switch (target) {
                case MathExpUtils.TARGET_NAME.GLOBAL: return globalMap;
                case MathExpUtils.TARGET_NAME.THEORY: return theoryMap;
                default: throw `Unknown target map ${target}`;
            }
        }
        this.targetOrder = (target) => {
            switch (target) {
                case MathExpUtils.TARGET_NAME.GLOBAL: return globalOrder;
                case MathExpUtils.TARGET_NAME.THEORY: return theoryOrder;
                default: throw `Unknown target map ${target}`;
            }
        }
        this.genHooks();
...
MathExpUtils.TARGET_NAME = { GLOBAL: 'GLOBAL', THEORY: 'THEORY' };
MathExpUtils.MODESET = { NONE: 'NONE', TICK: 'TICK', PRESTIGE: 'PRESTIGE', SUPREMACY: 'SUPREMACY', GRADUATION: 'GRADUATION', PUBLISH: 'PUBLISH' };
MathExpUtils.TARGET_NAME_STR = { GLOBAL: 'Global Variable', THEORY: 'Theory Variable' };
MathExpUtils.MODESET_STR = { NONE: 'never', TICK: 'every tick', PRESTIGE: 'every prestige', SUPREMACY: 'every supremacy', GRADUATION: 'every graduation', PUBLISH: 'every publication' };
MathExpUtils.internalMap = {};

// =====================================================================
// [3] AllocUtils (★ 및 σ 휴리스틱 기반 재분배 유틸리티 클래스)
// =====================================================================
class AllocUtils {
    static simpleStar() {
        ...
        for (let k = 0; k < MAX_ITER; k++) {
            ...
            for (const i of doubleUps) {
                ...
                if (best == null || best.dx * cost < dx * best.cost) {
                    best2 = best;
                    best = { isDouble: true, i: i, dx: dx, cost: cost, cnt: dumpLevels };
                } else if (best2 == null || best2.dx * cost < dx * best2.cost) {
                    best2 = { isDouble: true, i: i, dx: dx, cost: cost, cnt: dumpLevels };
                }
            }
            ...
            for (const i of singleUps) {
                ...
                if (best == null || best.dx * cost < dx * best.cost) {
                    best2 = best;
                    best = { isDouble: false, i: i, dx: dx, cost: cost, cnt: 1 };
                } else if (best2 == null || best2.dx * cost < dx * best2.cost) {
                    best2 = { isDouble: false, i: i, dx: dx, cost: cost, cnt: 1 };
                }
            }
...
class PopupUtils {
    static showAutoFreqPopup() {
        ...
        let apply = ui.createButton({ text: "Apply" });
    ...
    static showR9CyclePopup() {
        ...
        let entryX = ui.createEntry({
            ...
            text: recordX,
            onTextChanged: (_, s) => { recordX = s; }
        });
        let entryY = ui.createEntry({
            placeholder: "Y Ticks (No R9)",
            text: recordY,
        ...
        let popup = ui.createPopup({
            title: "R9 Cycle Configuration",
            content: ui.createStackLayout({
                children: [
                    ui.createLabel({ text: "Use R9 Cycle Mode?" }),
                    toggleSwitch,
                    ui.createLabel({ text: "X Ticks (Buy R9):" }),
                    entryX,
                    ui.createLabel({ text: "Y Ticks (No R9):" }),
                    entryY,
                    apply
                ]
            }),
...
// =====================================================================
// [5] State 클래스 및 인코더 매핑 (데이터 영구 저장 관리)
// =====================================================================
class State {
    constructor() {
        this.useR9 = false;
    ...
    importSerialization(s) {
        ...
        Object.keys(this).forEach(key => { if (key in intermediate) this[key] = intermediate[key]; });
        return this;
    }
}

var customReplacer = (_, val) => {
    try { if (val instanceof BigNumber) return "BigNumber" + val.toBase64String(); } catch {}
    return val;
}
var customReviver = (_, val) => {
    if (val && typeof val === 'string') { if (val.startsWith("BigNumber")) return BigNumber.fromBase64String(val.substring(9)); }
    return val;
}
var getInternalState = () => state.serialize();
var setInternalState = (s) => { if (s) state.importSerialization(s); }

// =====================================================================
// [6] Theory Automator 기초 연산 수식 및 공통 코어 함수
// =====================================================================
...
for (let i = 0; i < 8; i++) { quaternaryEntries.push(new QuaternaryEntry("τ_" + (i + 1), 0)); }
var getQuaternaryEntries = () => {
	let decay = [30.1935671759384, 37.4972532637665, 30.7608639120181, 44.9544911685781, 39.2687021300084, 102.119195226465, 26.7695950304505, 17.6476778516314];
	...
	for (let i = 0; i < Math.min(8, game.researchUpgrades[7].level); i++) {
		try { tau = game.theories[i].tauPublished.log10(); } catch(e) { tau = 1; }
		tauH = base[i] * R9 ** (1 / timeMult[i]) / 2 ** ((tau - requirements[i]) / decay[i]);
		quaternaryEntries[i].value = formatQValue(tauH);
	}
	for (let i = game.researchUpgrades[7].level; i < 8; i++) { quaternaryEntries[i].value = formatQValue(0); }
	if (game.researchUpgrades[7].level < 4) return quaternaryEntries;
	decay = 27.0085302950228; base = 1.51; timeMult = 1;
	...
	decay = 70.0732254255212; base = 7; timeMult = 2;
...
function refreshTheoryManager() {
	...
	else if (theoryId == 1) theoryManager = new T2;
	else if (theoryId == 2) theoryManager = new T3;
	else if (theoryId == 3) theoryManager = new T4;
	else if (theoryId == 4) theoryManager = new T5;
	else if (theoryId == 5) theoryManager = new T6;
	else if (theoryId == 6) theoryManager = new T7;
	else if (theoryId == 7) theoryManager = new T8;
...
// =====================================================================
// [7] 개별 이론 제어 오토메이터 로직 클래스군 (T1 ~ T8)
// =====================================================================
...
var tick = (elapsedTime, multiplier) => {
    ...
    if (game.activeTheory !== null) {
        ...
        if (theoryManager && theoryManager.tick(elapsedTime, multiplier))
            switchTheory();
    }
...
// =====================================================================
// [9] UI 동적 빌드 및 렌더링 결합 (getCurrencyBarDelegate)
// =====================================================================
// =====================================================================
// [원본 100% 유지] UIutils 클래스
// =====================================================================
class UIutils {
...
// =====================================================================
// [원본 유지 + 하단 QoL UI 주입] getUpgradeListDelegate 변수 함수
// =====================================================================
var getUpgradeListDelegate = () => {
    ...
    // -----------------------------------------------------------------
    // [추가 구역] 밑에 배치될 QoL 기능의 UI 컴포넌트 생성 정의
    // -----------------------------------------------------------------
    let qolSeparator = ui.createBox({
        heightRequest: 1
    });
    ...
    let r9Label = ui.createLabel({
        ...
        horizontalTextAlignment: TextAlignment.CENTER,
        textColor: () => state.useR9 ? Color.TEXT : Color.TEXT_MEDIUM
    });
    let r9Switch = ui.createSwitch({
        isToggled: () => state.useR9,
        onTouched: (e) => { if (e.type == TouchType.PRESSED) state.useR9 = !state.useR9; }
    });
    let r9toggleStack = ui.createStackLayout({
        children: [r9Label, r9Switch]
    });

    reStar.row = 0; reStar.column = 0;
    reSigma.row = 0; reSigma.column = 1;
    ...
    let r9CycleButton = ui.createButton({
...
// =====================================================================
// [10] 초기화 제어 (맨 밑의 블록을 아래와 같이 수정하세요)
// =====================================================================
{
    // 🔥 이 줄이 누락되어 오류가 발생했습니다. 반드시 추가해야 합니다.
    state = new State(); 
